...
\usepackage{babel} % Bibliographie et références
...
\begin{description}
    ...
    \item[$\tau {_C_O_2}$:] Coéfficient de transmission du CO2 (sans unité)
...
La luminance d'un corps noir est évalué grace à la loi de Planck (en $W.m^{-3}$): \\
...
Et ainsi l'exitance totale d'un coprs noir (en $W.m^{-2}$) est obtenue avec la formule suivante:
 ...
 \hfil M^{0}=\[ \int_{0}^{\infty} M^{0}_\lambda \,\mathrm{d}\lambda \]=$\sigma$\times$T^{4}$\par
...
\begin{center}
$\Phi_{ray}$=\int$\epsilon $\times $\sigma$ \times$ T^{4} dS $    
\end{center}
2. Numériquement\\
En effet évaluer le flux émit revient à calculer l'aire sous la courbe de son spectre d'émission. (voir Figure  "mettre nombre figure" et inserer le code pour faire l'intégrale sous la courbe) \\

Ainsi avec ces deux méthodes, on trouve que le rayonnement émis par le Soleil est de 174 PW  soit $174.10^{15} W$
