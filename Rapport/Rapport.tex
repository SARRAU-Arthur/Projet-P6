...
\begin{description}
	...
	\subsubsection*{Constantes \footnote{En Unités du Système International (USI)}}
	...
	\item[\boldmath{$h$}:] Constante de Plank = $6,63.10^{-34}$ $kg.m^2.s^{-1}$
 	\item[\boldmath{$kB$}:] Constante de Boltzmann = $1.380649^{-23}$ $kg.m^2.s^{-2}.K^{-1}$
  	\item[\boldmath{$g$}:] Constante gravitationelle terrestre = $9.80665$ $m.s^{-2}$
   	\item[\boldmath{$c0$}:] Célérité de la lumière = $2.99^8$ $m.s^{-1}$
    	\item[\boldmath{$C1$}:] Constante de Planck 1 = $2 *h * (c0 / n)^2$ $kg.s^{-3}.m^{-4}$
     	\item[\boldmath{$C1$}:] Constante de Planck 2 = $(h * c0) / (n * kB)$ $m.K$
      	\item[\boldmath{$Ts$}:] Temperature du soleil = $5.772^3$ $K$
	\item[\boldmath{$\bar{a_T}$}:] Albédo terrestre moyen $\approx$ $0.31$
	% Expliciter les constantes ci dessous comme l'on été celles du dessus en gardant 
	% la même mise en page avec les indices, unités et puissances écrits proprement en Latex 
