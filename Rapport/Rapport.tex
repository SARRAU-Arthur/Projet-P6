...
\begin{description}
	...
	\item[\boldmath{$k_{abs}$}:]: coefficient d'absorbition obtenue avec la base de donnée HITRAN ($m^2.molec^{-1}$)
...
D'après la loi des Gaz Parfaits, en utlisant le principe fondamentale de la statique des fluides on a: 
...
T(z)=$T_o$-az      
\end{center}
avec a= coefficient directeur, $T_o$ = Température de l'atmosphère initale et $T_f$ = Température de l'atmosphère finale \\

On peut à présent calculer la pression en fonction de l'altitude, en utilisant le principe fondemantale de la statique des fluides. 
\begin{center}
