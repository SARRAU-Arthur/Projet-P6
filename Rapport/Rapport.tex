...
Radiat. Transfer 177, 15-30 (2016)DOI: 10.1016/j.jqsrt.2016.03.005} 
...
\begin{description}
    ...
    \item[\boldmath{$\phi$}:] Densité de flux 
							\footnote{Dans ce rapport il décrira exclusivement la puissance 
    ...
    \item[\boldmath{$n(z)$}:] Quantité de matière de CO2 en fonction de l'altitude ($molec.m^{-3}$)
	...
	\item[\boldmath{$\bar{a_T}$}:] Albédo terrestre moyen = $3,10.10^{-1}$
...
\noindent L'émittance spéctrique d'un corps noir est évalué grâce à la loi de Planck \vspace{\baselineskip}
...
\begin{center}
    $\Rightarrow n(z)= \frac{P(z)}{k_BT} $ , où $k_B = \frac{R}{N_A}$
\end{center}   \vspace{\baselineskip}
...
On détermine C grâce aux conditions initiales de chaque modèle. Nous pouvons connaitre la pression de la 1er altitude de modèle $P(z_i)$ grâce au modèle précédent
...
\begin{center}
     ${P(z)} = {P(z_i)}{e^{-\frac{gM}{RT} (z_i-z)}}$
\end{center}


On a donc 
\begin{center}
	$n(z)=\frac{P(z)}{{k_B}T}= \frac{ P_(z_i) e^{-\frac{gM}{RT} (z_i-z) }}{{k_B}T}$
 \end{center}
...
On détermine C grâce aux conditions initiales de chaque modèle. Nous pouvons connaitre la pression de la 1er altitude de modèle $P(z_i)$ grâce au modèle précédent (ou pour le 1er modèle, en sachant que à z = 0km on a ${P_o}$ :
\begin{center}
...
\begin{center}
$P(z) = (az+b)^{\frac{-Mg}{Ra}} \times \frac{P(z_i)}{((a{z_i}+b))^{\frac{-Mg}{Ra}}}$
\end{center}

\begin{center}
$\Leftrightarrow P(z)={P(z_i)} (\frac{az+b}{(a{z_i}+b)})^{\frac{-Mg}{Ra}}$
\end{center}

On a donc :
\begin{center}
   $n(z)=\frac{P(z)}{{k_B}{T(z)}}= \frac{{P(z_i)}(\frac{a(z)z+b(z)}{(a(z){z_i}+b(z))})^{\frac{-Mg}{Ra(z)}}}{{k_B}{T(z)}}$
\end{center}
